...
async function generateSBOM(repoPath, options = {}) {
  ...
  for (const candidate of candidates) {
    try {
      if (candidate === 'syft') {
        if (!(await isCommandAvailable('syft'))) {
      ...
      if (candidate === 'trivy') {
        if (!(await isCommandAvailable('trivy'))) {
...
async function checkProjectEnvironment(repoPath) {
  ...
  const syftAvailable = await isCommandAvailable('syft');
...
async function runNpmAudit(repoPath) {
  const command = `cd "${repoPath}" && npm audit --json --package-lock-only`;
  const packageLockPath = path.join(repoPath, 'package-lock.json');
  const prepareLockCommand = `cd "${repoPath}" && npm install --package-lock-only --ignore-scripts --no-audit --fund=false`;
