  ...
  const cycloneDx = {
    ...
    metadata: {
      ...
      tools: [
        {
          vendor: 'CSAP',
          name: 'Code Structure and Security Analyzer',
          version: '1.0',
        },
      ],
...
function exportSBOMAsSpdx(sbom) {
  const lines = [
    ...
    `DocumentNamespace: https://csap/${generateUUID()}`,
    'Creator: Tool: CSAP-1.0',
...
async function runNpmAudit(repoPath) {
  ...
  try {
    ...
    return {
      ...
      vulnerabilities: [],
      fixable: 0,
      summary: 'npm auditの実行に失敗しました。依存関係のインストール状態を確認してください。',
...
async function scanVulnerabilities(repoPath, scannerPreference = 'auto') {
  ...
  if (!hasPackageJson) {
    return {
      ...
      vulnerabilities: [],
      fixable: 0,
      summary: 'package.json が見つからないため、Node.js依存関係の脆弱性チェックはスキップされました。',
    };
  }
...
function formatSummaryCounts(total, critical, high, moderate, low, fixable) {
  ...
  const fixableText = fixable > 0 ? `修正可能パッケージ:${fixable}` : '修正可能パッケージ:0';
  return `${total}件の脆弱性 (${parts.join(', ')}) / ${fixableText}`;
}

function formatVulnerabilitySummary(scanResult) {
  if (!scanResult) {
    return '脆弱性スキャン結果が取得できませんでした。';
  }
